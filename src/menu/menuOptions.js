...
import {
  blockTypeIsActive, cmdItem, markItem, toggleBlockType,
} from './common';
import icons from '../icons';
import { markActive } from '../utils';

const wrapListItem = (nodeType, options) => cmdItem(
  wrapInList(nodeType, options.attrs),
  options,
);

const imageUploadItem = (nodeType, onFileUpload) => new MenuItem({
  title: 'Upload image',
  icon: icons.image,
  enable() {
    return true;
...
const buildMenuOptions = (schema, {
  enabledMenuOptions = ['strong', 'em', 'link', 'bulletList', 'orderedList', 'undo', 'redo', 'code'],
  onFileUpload = () => { },
}) => {
  const availableMenuOptions = {
    ...
    imageUpload: imageUploadItem(schema.nodes.image, onFileUpload),
  };
